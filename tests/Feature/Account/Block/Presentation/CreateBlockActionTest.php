...
final class CreateBlockActionTest extends TestCase
{
    use RefreshDatabase;

    public function test_returns_unauthorized_without_an_authenticated_account(): void
    {
        $this->postJson('/api/blocks', $this->validPayload())
            ->assertUnauthorized()
            ->assertSeeText('');
    }

    public function test_creates_a_block_for_the_authenticated_account(): void
    {
    ...
    public function test_returns_conflict_for_a_duplicate_block_with_exact_json(): void
    {
        $this->insertAccount('3b5581e9-16df-4879-b7d2-5d88dca6ab87', 'blocking@example.com');
        $this->insertAccount('f0cfa1a3-1ac7-44af-9bf4-b36c9262f028', 'blocked@example.com');
        $this->authenticatedRequest()->assertNoContent();
        ...
        $this->authenticatedRequest()
            ->assertConflict()
            ->assertExactJson(['message' => 'すでにブロック済みです。']);
    }

    public function test_returns_unprocessable_for_a_self_block_with_exact_json(): void
    {
        $this->insertAccount('3b5581e9-16df-4879-b7d2-5d88dca6ab87', 'blocking@example.com');

        $this->authenticatedRequest([
            'blocked_account_identifier' => '3b5581e9-16df-4879-b7d2-5d88dca6ab87',
        ])
            ->assertUnprocessable()
            ->assertExactJson(['message' => '自分自身をブロックすることはできません。']);
    }

    public function test_returns_not_found_for_a_missing_blocked_account_with_exact_json(): void
    {
        $this->insertAccount('3b5581e9-16df-4879-b7d2-5d88dca6ab87', 'blocking@example.com');
    ...
    public function test_validates_the_block_request(): void
    {
        $this->withSession(['account_identifier' => '3b5581e9-16df-4879-b7d2-5d88dca6ab87'])
            ->postJson('/api/blocks', ['blocked_account_identifier' => 'invalid'])
            ->assertUnprocessable()
            ->assertJsonValidationErrors(['blocked_account_identifier']);
    }
    ...
    private function authenticatedRequest(array $overrides = []): TestResponse
    {
        return $this->withSession(['account_identifier' => '3b5581e9-16df-4879-b7d2-5d88dca6ab87'])
            ->postJson('/api/blocks', $this->validPayload($overrides));
    }
