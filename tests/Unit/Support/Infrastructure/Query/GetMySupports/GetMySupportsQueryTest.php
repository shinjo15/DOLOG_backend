...
use Src\Support\Application\Usecase\Query\GetMySupports\GetMySupportsItemOutputPort;
...
final class GetMySupportsQueryTest extends TestCase
{
    ...
    public function test_it_returns_only_my_action_supports_in_descending_support_datetime_order(): void
    {
        ...
        $result = (new GetMySupportsQuery)->execute(new GetMySupportsInput('33333333-3333-4333-8333-333333333333', 1, 10));
    ...
    public function test_it_paginates_my_action_supports(): void
    {
        ...
        $result = (new GetMySupportsQuery)->execute(new GetMySupportsInput('33333333-3333-4333-8333-333333333333', 2, 2));

        self::assertSame(3, $result->total());
        self::assertSame(2, $result->currentPage());
        self::assertSame(2, $result->lastPage());
        self::assertSame(2, $result->perPage());
        self::assertSame(['aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'], array_map(static fn (GetMySupportsItemOutputPort $item): string => $item->postIdentifier(), $result->items()));
    }
     ...
     */
    private function items(GetMySupportsOutputPort $result): array
    {
        return array_map(static fn (GetMySupportsItemOutputPort $item): array => [
            'postIdentifier' => $item->postIdentifier(),
            'routineIdentifier' => $item->routineIdentifier(),
            'postCategory' => $item->postCategory(),
            'postLikeCount' => $item->postLikeCount(),
            'postSupportCount' => $item->postSupportCount(),
